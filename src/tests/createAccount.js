...
export const createAccount = async (driver) => {
    ...
    const confirmPasswordInput = await driver.findElement(By.id("confirm-password-input"))
    await confirmPasswordInput.sendKeys(process.env.COGNITO_PASSWORD)
    await wait(driver)

    const signUpButton = await driver.findElement(By.id("sign-up-button"));
    ...
    const codeInput = await driver.findElement(By.id("code-input"))
    await codeInput.sendKeys(code)

    const confirmAccountButton = await driver.findElement(By.id("confirm-registration-button"))
